...
if(count($errors) === 0){
	try{
		...
		//アカウントで検索
		$statement = $dbh->prepare("SELECT * FROM member2 WHERE account=(:account)");//flag=1抜いた
		...
		if($row = $statement->fetch()){
			...
			if (password_verify($password, $password_hash)) {
				...
				$errors['password'] = "エラー1アカウント及びパスワードが一致しません。";
			}
		}else{
			$errors['account'] = "エラー2アカウント及びパスワードが一致しません。";
		}
